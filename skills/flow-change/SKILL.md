---
name: flow-change
description: 变更提案生成器。把用户的模糊想法通过反问澄清为一份变更提案 CHANGE.md，自动生成 change-id，检测架构级变更（优先使用 GitNexus MCP impact 分析），前端项目预选视觉调性，判定影响面并给出路径建议。Use when 用户说"我想做/加/实现/设计某事"且无活跃 change 时，或需要发起新变更提案时。
---
...
**GitNexus 路径（优先）**：对 CHANGE.md 涉及的关键符号调用 `gitnexus_impact({target: "核心类/模块名", direction: "upstream", maxDepth: 3})`：
