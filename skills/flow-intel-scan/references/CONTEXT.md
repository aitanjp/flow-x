...
> 这里写**全项目共用**的栈。每次 CHANGE 的 `DESIGN.md ## 0` 会读此处作为默认；如果某次 CHANGE 用了不同的栈，那次 DESIGN.md ## 0 会显式覆盖。
...
- **栈卡片编号**（来自 `tech-stacks.md`）: <如 1 Next.js 全栈>
...
> 只记 Scheduled 和未处理项。Critical 已通过 health-fix CHANGE 处理中，不在此列。
