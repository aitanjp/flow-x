...
description: 集成验证与归档执行器。跑全套自动化测试，引导人工 UAT，失败诊断并产 fix-plan（最多 3 轮自动重试），提名 LESSONS 到知识库，归档 change 到 archive/ 目录并更新 CHANGELOG.md 和 STATE.md。提交前用 GitNexus MCP 检测变更影响范围。Use when 用户确认 REVIEW.md 通过后需要进入集成验收阶段，或需要 UAT/归档/发布时。
---
...
**提交前检测**：归档前必须调用 `gitnexus_detect_changes(scope="all")` 验证本次 change 只影响预期符号和执行流。返回 HIGH/CRITICAL risk → 必须回 flow-review 重新审查。
