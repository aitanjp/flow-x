...
> 注入后你只需要 `@flow-x/prompts/<n>-*.md` 引用当前阶段，不用再 `@METHODOLOGY` 和 `@RULES`。

---

## 你正在协作一个使用 flow-x 流程的项目
...
- `CONTEXT.md` — 域语言 + 默认决策（术语表 / 已锁决策 / 偏好）
- `DESIGN.md` — 技术设计（架构图 / 数据流 / ADR / 风险）
- `UI-DESIGN.md` — UI 美学方向 + design tokens（OKLCH 颜色 / 字体 / 间距 / 动效）+ 反 AI-slop 自检（仅前端项目）
- `TASK.md` — 任务清单（任务 / 依赖 / `[P]` / verify / done）
- `TEST.md` — 5 轮测试金字塔报告（功能矩阵 + UAT + 性能 / 安全 / 兼容 / 可观测口供）
- `REVIEW.md` — 审查发现（严重度 / 修复决策 / 跨模型分歧）
- `SUMMARY.md` — 阶段产物快照（用于截断历史）
- `<task-id>-PROGRESS.md` — **临时**文件——任务执行中途清窗时写入，含「已排除方案」反重复段。任务完成后删除。
- `LESSONS.md`（`.specs/` 根）— **项目级常驻**——跨 change 失败知识库。每个 DEV 任务开工前必扫；INTEGRATION 阶段提名新条目。
- `STATE.md`（仓库根）— 跨会话状态（当前位置 / 中断任务 / 阻塞 / 决策日志）
...
> 完整方法论与机制详见 `METHODOLOGY.md`，本文件是其精简注入版。
