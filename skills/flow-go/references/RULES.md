...
- **R1.5 · 重启协议（清窗前后必须严格执行）**
  ...
  **清窗前** AI 必须依次完成：
  1. 把当前任务的中间状态写入 `.specs/<id>/<task-id>-PROGRESS.md`。必须包含：已完成子步骤 / 当前正在做 / **已排除方案及理由** / 待确认假设。
  2. 更新仓库根 `STATE.md` 的「中断任务」字段。
  3. 输出一段"重启指令"给用户。

  **清窗后** AI 接手时必须按以下顺序加载：
  `METHODOLOGY → RULES → 当前阶段 prompt → CONTEXT → REQUIREMENT → DESIGN（≥阶段2时）→ TASK（≥阶段3时）→ <task-id>-PROGRESS（中途断的）`
...
- **R1.7 · 任务过大早期信号** 一个 task 在执行到一半就触发 R1.1 清窗 → 说明该 task 在 TASK 阶段就拆得不够细。恢复后第一动作不是继续干，而是把它在 `TASK.md` 里**就地拆为 ≥ 2 个子任务**。
...
- **R1.8 · 跨任务失败检查（任何 DEV 任务进入实现前必跑）**
  ...
  3. 若计划方案与某条 active 条目**完全相同** → 触发 R1.6，禁止直接重试
...
- **R1.9 · 工件加载预算（任何阶段首轮强制）**
  ...
  | REFERENCE | `flow-x/reference/*.md` | 75~470 行 | **禁止默认整读**，按节 grep / read offset |
  | TEMPLATE / PROMPT | `flow-x/templates\|prompts/*.md` | < 150 行 | 整读 OK |
...
- **R4.5 · Schema 变更必伴随迁移文件（强制）**
  ...
  1. **必须**同次提交附带迁移文件
  2. **必须**在执行计划里列出 schema diff
  3. **必须**生成可逆迁移（含 `up` / `down`）
  4. **必须**检测项目有无 DB 凭据：
     - 找到凭据 → **反问用户**「是否现在执行迁移」
     - 未找到凭据 → 生成 SQL 文件 + 在 `SUMMARY.md`「数据库迁移」段**显式提醒**
  5. **禁止**只改 model 不写迁移就提交

- **R4.6 · 破坏性变更高门槛（强制）**

  AI 在 4-dev 阶段命中下面**任一**条件时，禁止直接动手，必须先走「破坏性变更协议」：

  1. **删除既有代码** ≥ 5 行（不算空行/注释）
  2. **改公共导出**：导出函数 / 类 / 接口的签名变更
...
- **R5.4** TEST.md 必须声明 5 轮金字塔（功能 / 性能 / 安全 / 兼容 / 可观测）的状态。跳过任意一轮必须给理由。
- **R5.5** 性能 / 安全 / 兼容轮次的"通过 / 失败"必须基于**可量化指标**或**工具输出**，禁止"看起来良好"、"基本没问题"等空话。
...
- **R6.4 · 沿用既有抽象（强制）**
  ...
  **必须**在 `SUMMARY.md`「6 维自查」段贴 grep 命令和结果。
...
- **R6.5 · 提交前 diff 边界 verify（强制）**
  ...
  4. 把 verify 结果贴入 SUMMARY「越界检查」段

## R7 · 范围控制

- **R7.1** 严禁在执行任务时悄悄扩大范围。发现需要超出 `TASK.md` 的改动时，必须先停下来更新 TASK 或开新 CHANGE。
- **R7.2** 不允许同一次提交里混入多个无关任务。
- **R7.3** TASK.md 每个任务必须声明 `read_files`（参考边界）和 `write_files`（修改边界）两组路径。
